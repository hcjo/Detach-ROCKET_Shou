...
class DetachRocket:
    ...
    def estimate_channel_relevance(self):
        ...
        feature_weights = self._feature_importance_matrix[self._max_index] # Sparse float array (num_features,)
        selection_mask = feature_weights > 0

        channel_combinations_derocket = self._full_transformer.get_kernel_features('channels', selection_mask) # Indicator matrix (num_features, num_channels)
